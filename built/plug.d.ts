...
}>(stateMapper?: (state: Red["initialState"]) => StateProps, dispatchMapper?: (dispatcher: Redutser.Dispatcher<Red["actionTypes"], Red["initialState"]>) => DispatchProps) => ComponentEnhancer<StateProps & DispatchProps & OwnProps, OwnProps>;
export declare const plug: <Red extends Redutser<any, any>>(redutser: Red) => {
    ownProps: <OwnProps = {}>() => {
        mapProps: <StateProps = Red["initialState"], DispatchProps = {
            dispatch: Redutser.Dispatcher<Red["actionTypes"], Red["initialState"]>;
        }>(stateMapper?: (state: Red["initialState"]) => StateProps, dispatchMapper?: (dispatcher: Redutser.Dispatcher<Red["actionTypes"], Red["initialState"]>) => DispatchProps) => {
